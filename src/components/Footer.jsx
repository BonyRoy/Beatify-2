...
const Footer = () => {
  ...
  const [shouldScrollFullscreenArtist, setShouldScrollFullscreenArtist] =
    useState(false);
  ...
  const handleFooterClick = (e) => {
    ...
    if (isFullScreen) {
      ...
      if (
        e.target.closest(".player__center") ||
  ...
  useEffect(() => {
    ...
    const iosBackgroundMonitor = () => {
      ...
      if (
        ...
        audio.paused &&
        !isSourceChangingRef.current &&
        timeSinceTrackChange > 1000
      ) {
        // Track changed, should be playing - try multiple times
        if (
          !audio.dataset.playAttempted ||
          now - parseInt(audio.dataset.playAttempted || "0") > 2000
        ) {
                  ...
                  audio
                    ...
                    .catch((error) => {
                      ...
                      if (attempt < 5) {
                        ...
                        console.log(
                          "iOS background play failed after max attempts",
                        );
                      }
                    });
                } else {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (
        showSpeedMenu &&
        !e.target.closest(".player__fullscreen-speed") &&
        !e.target.closest(".player__speed")
      ) {
        setShowSpeedMenu(false);
      }
    };
    ...
    const checkOverflow = () => {
      ...
      requestAnimationFrame(() => {
        setTimeout(
          () => {
            ...
            if (albumRef.current) {
              ...
              if (contentEl && innerEl) {
                const isOverflowing =
                  albumEl.scrollWidth > contentEl.offsetWidth + 5;
            ...
            if (fullscreenArtistRef.current && isFullScreen) {
              ...
              const wrapperEl = artistEl.closest(
                ".player__fullscreen-artist-wrapper",
              );
              const innerEl = artistEl.closest(
                ".player__fullscreen-artist-inner",
              );
              if (wrapperEl && innerEl) {
                const isOverflowing =
                  artistEl.scrollWidth > wrapperEl.offsetWidth + 5;
          ...
          isFullScreen ? 300 : 200,
        ); // Longer delay for fullscreen to ensure DOM is ready
      });
    };
              ...
              <div className="player__left">
                ...
                <div className="player__info">
                  <div className="player__title-wrapper">
                    ...
                    >
                      <p ref={titleRef} className="player__title">
                        {currentTrack.name || "Unknown Track"}
                      </p>
                      {shouldScrollTitle && (
                  ...
                  <div className="player__artist-wrapper">
                    ...
                    >
                      <p ref={artistRef} className="player__artist">
                        {currentTrack.artist || "Unknown Artist"}
                      </p>
                      {shouldScrollArtist && (
                    ...
                    <div className="player__album-wrapper">
                      <div className="player__album-content">
                        <div
                          ...
                          style={
                            shouldScrollAlbum
                              ? { "--scroll-duration": `${albumDuration}s` }
                              : {}
                          }
                        >
                          <span ref={albumRef} className="player__album-text">
              ...
              <div className="player__right">
                ...
                {!isMobile && (
                  <div className="player__speed">
                    ...
                    {showSpeedMenu && (
                      <div
                        className="player__speed-menu player__speed-menu--footer"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {speedOptions.map((speed) => (
                          <button
